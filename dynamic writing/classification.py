...
Z = np.array(df_Z)
df_BLANK = pd.read_table('letter/blank.txt',header=None,sep=',')
BLANK = np.array(df_BLANK)
...
class_blank = [26 for i in range(len(BLANK))]
...
y_label = np.append(y_label,class_blank)
y_label = y_label.reshape(5345)
...
with open('X_train, X_test,y_train, y_test.pickel', 'wb') as f1:
    pickle.dump([X_train, X_test,y_train, y_test], f1)

clf =svm.SVC(gamma='scale',
             C=1.2, decision_function_shape='ovr',
             kernel='rbf',
             probability=True)
...
def plot_confusion_matrix(confusion_mat):
    ...
    plt.title('Confusion Matrix of Dynamic Writing',fontsize = 12)
    plt.colorbar()
